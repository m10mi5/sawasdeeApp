...
| 17 | เก | – | สระ เอ | Sara Ee | ee | Long | เล่น | to play | ✓ |
...
14. **Vowel deck mirrors consonant card** — `Vowel` cards reuse the same CSS classes as consonant cards; `LENGTH_COLORS` maps Short/Long/Diphthong to the same blue/green/orange palette as CLASS_COLORS. The `symbol` field always includes ก as the base consonant so the vowel form is visible. The main large text (`.character`) shows the `exampleWord` so learners see a real Thai word first. Below it, supporting details show: romanisation = meaning, vowel symbol / closed symbol, Thai name · romanised name, romanisation, and length — all hidden until Show Details is toggled. Play button speaks the `exampleWord` (with recorded audio when available).
